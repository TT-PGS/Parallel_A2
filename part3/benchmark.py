...
class TestParallelAStar(unittest.TestCase):
    ...
    def run_astar_version(self, name, version, threads):
        results = []
        image_dir = os.path.join(OUTPUT_BASE_DIR, "part3", f"run_with_{threads}_threads", "images")
        ...
        for (name1, coord1), (name2, coord2) in self.point_pairs:
            ...
            path, f_vec = astar_parallel_solver(
                node1, node2, self.graph, dynamic_h, f_vector_basic,
        ...
        result_dir = os.path.join(OUTPUT_BASE_DIR, "part3", f"run_with_{threads}_threads")
        save_results(results, result_dir, f"benchmark_{name}")

    def test_parallel_astar(self):
        for version in VERSIONS:
            for threads in THREAD_COUNTS:
                algo_id = f"part3_{version}_{threads}t"
                self.run_astar_version(algo_id, version, threads)
